...
app.use(session({
  ...
  name: 'antrax_session' // Custom session cookie name
}));
...
const requireCheckerAccess = (req, res, next) => {
  if (req.session && req.session.user && req.session.user.canCheck) {
    return next();
  }
  if (req.path.startsWith('/api/')) {
...
app.get('/login', (req, res) => {
  console.log('[LOGIN] Page requested, session:', req.session);
  if (req.session && req.session.user) {
...
app.get('/api/auth/status', (req, res) => {
  ...
  console.log('[AUTH STATUS] User:', req.session?.user);
...
app.get('/', requireAuth, (req, res) => {
  console.log('[DASHBOARD] Rendering for user:', req.session.user);
...
app.post('/api/batch', requireAuth, requireCheckerAccess, async (req, res) => {
  ...
  try {
    ...
    const options = { enrichValid: enrichValid || false, checkBan: checkBan || false };
...
╔═══════════════════════════════════════════════════════════════════════╗
║                                                                       ║
║        ANTrax ML CHECKER v3.0                                        ║
║                                                                       ║
║        URL: http://localhost:${PORT}                                  ║
║        Login: /login                                                 ║
║        Admin: antrax / admin@AntraxdevZ                             ║
║                                                                       ║
║        API Endpoints:                                                ║
